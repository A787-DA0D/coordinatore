...
# ======================
# MOCK /tick (disabilitato, ma lasciato per retrocompatibilità)
# ======================

@app.post("/tick", response_class=PlainTextResponse, include_in_schema=False)
async def tick():
    try:
        PROJECT = _project_id()
        if not PROJECT:
        ...
        # Non usiamo più Cloud Tasks per questo; la Coscienza v3 parlerà con /v1/trade-intent.
...
# ======================
# ARBI BLOCK (già usato)
# ======================

@app.get("/arbi/block", include_in_schema=False)
def arbi_block():
    # LEGACY DEFI DISABLED
    raise HTTPException(status_code=410, detail="legacy endpoint disabled")

    if os.getenv('ENABLE_LEGACY_DEFI','0') != '1':
        raise HTTPException(status_code=410, detail='legacy defi endpoint disabled')
...
@app.get("/pyth/price", include_in_schema=False)
...
# ======================
# 🔥 /v1/trade-intent – cuore dell’autotrader
# ======================
...
async def v1_trade_intent(
    ...
    if COORD_API_KEY:
        if not x_api_key or x_api_key != COORD_API_KEY:
            logging.error("API KEY MISMATCH: header=%r expected=%r", x_api_key, COORD_API_KEY)
            raise HTTPException(status_code=401, detail="invalid api key")

    # 2) Validazione simbolo (universo CeFi)
    symbol = intent.symbol.upper()
    allowed = getattr(executor, "COSCIENZA_SYMBOLS_V3", None)
    ...
    # 4) Serializza intent in dict “piatto”
    intent_dict: Dict[str, Any] = intent.model_dump()
    intent_dict["symbol"] = symbol

    # (compat) router_cefi usa tenant resolution via user_id/email
    if not intent_dict.get("tenant_email"):
        intent_dict["tenant_email"] = intent_dict.get("user_id")
    ...
    try:
        ...
        return {"intent_id": None, "status": "ERROR", "message": str(e)}
...
async def reconcile_positions(
    ...
    """
    Reconciler istituzionale (VERITÀ BROKER):
    ...
    - chiude in DB le posizioni OPEN non più presenti sul broker
            ...
            for bp in open_positions:
                ...
                if res.rowcount == 0:
                    conn.execute(text("""
                        ...
                        VALUES
                          (:tenant_id, :symbol, :side, :size_usdc, :entry_price, 'OPEN', :opened_at,
